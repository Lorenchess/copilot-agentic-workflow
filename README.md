...
- **Phase 2 reference closure: recorded** at `5a46eb7` (fix-1; tag `phase-2-reference`). The independent targeted-correction review found F1–F3 closed and no material regression (contract §9). Accepted limitations (procedural recovery and approval controls, procedural Planner/Adversary independence, fictional examples) and the deferred corporate validation carry forward unchanged; the agent-picker smoke check remains **pending**. Phase 3 is not started.
