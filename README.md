...
docs/
  ...
  examples/PAYMENTS-12345-testing/         Phase 3 LARGE test-stage example on the same scenario, stops after stage 5b
  examples/PAYMENTS-12410/                 Phase 2 planning + Phase 3 test stages, SMALL, stops after stage 5b (planning-only version at tag phase-2-reference)
...
- **Phase 2 reference closure: recorded** at `5a46eb7` (fix-1; tag `phase-2-reference`). The independent targeted-correction review found F1–F3 closed and no material regression (contract §9). Accepted limitations (procedural recovery and approval controls, procedural Planner/Adversary independence, fictional examples) and the deferred corporate validation carry forward unchanged; the agent-picker smoke check remains **pending**. Phase 3 is delivered through P3-C3, below — not yet locked as a reference.
...
- **P3-C3 — Guide, README, closure: delivered.** `docs/COMPARISON-GUIDE.md` and this README updated; the sixteen challenge cases traced as document scenarios in the Phase 3 contract's §7 and the closure recorded in its §10 (Fable). Phase 3 is **not yet locked as a reference**: the assembled Phase 3 awaits its final independent review; push and lock remain separately authorized; the agent-picker smoke check remains **pending**; Phase 4 is not started.
