...
**Status: EXPERIMENTAL, off by default.** `.rstack/learnings.md` is optional repository-specific memory. No demonstrated run has shown that it saves more than it costs, so nothing in the pipeline requires it, reads it by default, or behaves differently when it is absent. A team turns it on by deciding to, in its adoption record; `discovery-cost.md` holds the test for whether it earned its place.
