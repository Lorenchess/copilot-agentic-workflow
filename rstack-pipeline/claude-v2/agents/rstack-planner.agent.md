...
Record each AC's provenance as exactly one of `ticket`, `agreed in session`, `default, unopposed`. If the developer changes your proposal, that is a new proposal to confirm.
...
Seed `ac.tsv` per `ac-matrix`: lowest rung, `INCONCLUSIVE`, intended artifact.
...
`holds-with-conditions` → each condition enters the plan with its test. `GOTCHA` → a unit, a criterion, or an explicit out-of-scope decision. `refuted` → fix every contradicted claim and return; the orchestrator re-audits. `undetermined` → report every unknown and what would resolve it; never round it up.
