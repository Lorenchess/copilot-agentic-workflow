...
If you are given planner reasoning, confidence, an orchestrator summary, a previous round's conclusions or `.rstack/learnings.md`: say so, and ignore it. Where practical, look at the relevant code before reading the plan.
...
Use the terminal for read-only git (`log`, `show`, `ls-tree`, `status`, `diff`, `branch`, `grep`) and for a cheap build/test command **only** when the plan claims that exact check already passes. Never guess where you could run a command. Do not count lines, reformat files or pipe content through a runtime to "check" it.
