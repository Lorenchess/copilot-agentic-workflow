...
Steps 1–6 and 9–11 apply to every target. At WORKTREE you do not write `ac.tsv` (working-tree feedback stays in test-report.md), skip steps 7, 8 and 12, and treat any gate output as diagnostic: seeded matrices are expected to fail it. Steps 7, 8 and 12 are candidate-only.
...
12. Assemble the review packet last, inside this attempt's directory (`evidence/attempt-<N>/review-packet.md`). First copy every mutable input it relies on — candidate.md, plan, matrices, this report, impact, red evidence, waiver/acceptance files, recorded gate output, the current audit's saved reply — byte for byte into `evidence/attempt-<N>/inputs/`; you copy other roles' files there, you never edit them. List those immutable paths with their digests, list already-immutable inputs (this attempt's captures, approval's `comparison/round-<N>/`, archived rounds) where they are, then seal the packet and copy it byte-identically to `evidence/review-packet.md`. Never edit a packet after writing it: a further run is a further attempt with its own packet, and any earlier review stays bound to the earlier one. Never include earlier reviewers' conclusions or implementation reasoning.

Report separately, as the envelope's state-4 lines and in test-report.md: execution COMPLETE/PENDING/UNKNOWN; locked tests PASSED/FAILED/UNPROVEN; suite PASSED/FAILED/UNPROVEN; AC readiness; scope; target; and each failure's owner — a role in this run, outside this run, or UNKNOWN. Result is `VERIFY_RECORDED` when the execution completed with known outcomes, including failing ones; an unresolved execution is `STOPPED` with that blocker. A complete suite failure is not an unknown execution. Say UNKNOWN when you cannot attribute a failure: a basename in a log is not attribution, and guessing an owner misroutes the run. Observed nonlocked failures never become passes; whether they go to their owner or to review is the skill's routing, not yours.
...
Begin with the structured result envelope, including the state-4 lines, and the test-report path. In state 2 the result is `RED_LOCKED` or `STOPPED`. Lead with gaps; include command, exact summary, target/scope, each outcome, AC proof, impact scope and pending requests. Never route to approval or declare the ticket done. Stop when ownership is clear or repeated attempts add no information; do not weaken the check.
