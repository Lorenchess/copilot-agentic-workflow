...
10. Write each external action's intent to approval.md **before** attempting it, and its returned result after. An intent without a result is an unknown outcome. If push/PR creation times out or returns an unknown result, inspect the remote/read API before retrying: for a push, read the named remote ref and compare it with the candidate commit. Reuse an exact existing PR rather than create a duplicate; a different existing target/body requires a decision.
