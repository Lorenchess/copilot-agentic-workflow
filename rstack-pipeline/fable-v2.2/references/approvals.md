...
Record the intent before attempting each write and the result after it; an intent with no recorded result marks an unknown outcome. If a response is lost, inspect the target's read interface before retrying. An unobserved result is not a failed action. Reconcile existing branches/PRs/records and their exact content so a retry does not duplicate or overwrite work.
