...
| Label | Meaning |
|---|---|
| `OBSERVED` | measured by a source independent of the agents being measured: host spans or session records, a tool measurement with inspected provenance for that particular field (tool authorship alone is insufficient), a human with a clock |
...
| ACs mapped to locked tests | matrix (agent-written) + lock (tool-written) | `acsMappedToTests` | `AGENT_REPORTED` for an agent-supplied AC mapping; a file hash is a byte measurement only if its capture is established, not evidence the mapping is correct |
