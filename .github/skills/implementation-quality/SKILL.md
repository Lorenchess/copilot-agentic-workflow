...
**Lost or incomplete execution.** A lost contract run, full-suite run, or scoped relied-on run is recorded `UNRESOLVED (<state>)` under its heading; no verdict is rendered for that repository. The round is not a FAIL round for `VERIFIER_FAIL_LIMIT`. Re-entry after the decision is a continuation of the pending verification obligations, not a single replacement run: the Verifier first re-establishes that repository's eligibility exactly as the initial preflight does — current review Basis, active anchor and membership, exception coverage, clean checkout at the same candidate SHA, and committed and working-tree controlled-path and envelope eligibility. A changed basis, anchor, membership, or checkout takes the existing invalidation and FAIL routes, never the replacement. Earlier `OBSERVED` results in that repository are retained only while their execution and basis remain applicable (same candidate SHA, review basis, anchor, and membership); confirmed failures are always retained. It then executes the authorized replacement and every still-unstarted required execution — the other of (A)/(B), any scoped relied-on run, and any repository not yet processed — inside the **same** verification round, then the post-execution bracket applies unchanged (the post-execution status must be clean and HEAD unchanged, otherwise FAIL "checkout changed during verification") and the verdict rules unchanged. No retry allowance is created; a further lost observation in the continuation returns to the hold with its own decision. The Verifier never checks out another revision to diagnose a hang, and never assumes Developer's diagnostic role.
