...
- `chat.tools.terminal.autoApprove` `true` rules are fully anchored (`^…$`), carry no `i` flag, and use value slots that cannot start with `-` so no flag can be smuggled into a slot. The `<dir>` slot is `[A-Za-z0-9][A-Za-z0-9._-]*` — it must start with a letter or digit, which is why `.` and `..` cannot match: `git -C .` or `git -C ..` would otherwise let an allowlisted verb operate outside the intended repository. The `<pbranch>` (pipeline branch shape) slot is `[A-Z][A-Z0-9_]*-[0-9]+-[A-Za-z0-9._-]+` — it matches only the `<PRIMARY>-<slug>` shape the pipeline itself creates (primary Jira key, then a numeric component, then a slug), so a branch-creating or branch-pushing rule cannot be satisfied by an arbitrary or pre-existing branch name such as a default branch.
...
The only path around all three is the change-request path: the developer records a `TEST-CHANGE-REQUEST` in IMPLEMENTATION.md, the pipeline STOPs for a developer decision, and — only on approval — the tester (never the developer) amends TEST-CONTRACT.md with the new RED commit, the new active protected paths, and the new active anchor, then re-runs the contract command and records the observed result per amended test (PASS, or RED with the failing assertion — a test that passes against the current tree is recorded PASS and is never labelled RED).
