...
Test integrity rests on three complementary controls (contract A4); none of the three alone is integrity:

1. **Byte-identity of protected paths at the active anchor.** TEST-CONTRACT.md records, per repository, the test file paths and the RED commit SHA, and — after any approved amendment — the new active protected paths and active anchor SHA. The verifier runs `git -C <dir> diff --stat <anchor>..HEAD -- <paths>` for each repository, anchored at the latest amendment when one exists and otherwise at the top-level RED commit; any change to those paths since the anchor that is **not** covered by an approved amendment recorded in TEST-CONTRACT.md is a FAIL.
2. **Execution-envelope review.** TEST-CONTRACT.md also records the execution envelope per repository: the exact contract command, the build/test configuration files, and the helper/fixture paths the tests depend on. The developer may change an envelope file only when the implementation genuinely needs it, with the change listed and justified in IMPLEMENTATION.md's Envelope changes; the verifier diffs the envelope files since the active anchor (`git -C <dir> diff --name-status <anchor>..HEAD -- <envelope files>`) and treats an unjustified change, or one that causes a contract test to be undiscovered, skipped, or excluded, as a FAIL.
...
The only path around all three is the change-request path: the developer records a `TEST-CHANGE-REQUEST` in IMPLEMENTATION.md, the pipeline STOPs for a developer decision, and — only on approval — the tester (never the developer) amends TEST-CONTRACT.md with the new active protected paths and the new active anchor, then re-runs the contract command and records the observed result per amended test (PASS, or RED with the failing assertion — a test that passes against the current tree is recorded PASS and is never labelled RED).
