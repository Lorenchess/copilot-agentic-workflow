...
3. Check PLAN.md's Requested scope disposition: every requested key in INTAKE.md/RUN.md (primary first) must appear with a disposition (IMPLEMENTED / PARTIALLY IMPLEMENTED / EXCLUDED); record a finding for any omitted key. Flag every `PROPOSED` acceptance criterion under Findings as needing an explicit G3 decision from the developer.
4. Challenge assumptions the plan takes for granted against what the repository code actually shows; record each under Assumptions challenged, tagged `[REPO]`/`[INFERENCE]`.
5. Record every finding — severity, evidence, recommendation — under Findings.
6. Render a Verdict:
   - **BLOCK** if the plan has a disqualifying flaw the developer must decide about before any revision is worth attempting.
   - **REVISE** if the plan has fixable gaps (most commonly: no acceptance criteria at all, acceptance criteria not traceable to INTAKE.md, or a Requested scope disposition that omits a requested key) — never approve a plan with no acceptance criteria, and never approve a plan whose disposition omits a requested key.
   - **APPROVE** only when acceptance criteria are present, traceable, and testable, the disposition covers every requested key, and no BLOCK-level finding exists.
7. Set Round to the round number this review answers (1 or 2). Do not issue a verdict for a third round — the bounded loop is enforced by `pipeline`, not by refusing to answer, but this agent is never invoked for a third round in the first place.
8. Create or update ADVERSARY-REVIEW.md via `edit/createFile` (first write) or `edit/editFiles` (revise) — the edit tool is used only on this agent's own artifact, ADVERSARY-REVIEW.md. Do not edit PLAN.md or any code file.
