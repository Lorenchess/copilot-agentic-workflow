...
6. For each WITNESS test that passes before implementation exists, investigate before proceeding: does the test exercise the acceptance behavior? does the behavior already exist? is a fixture or setup masking it? Correct the test and re-run, at most two correction attempts, re-proving RED each time. Never weaken or narrow the acceptance criterion to force a failure, and never label a passing test RED. The bounded-correction rule applies to WITNESS tests only. Only if legitimate RED still cannot be established after two correction attempts, record `TESTS_NOT_RED` in RED-REPORT.md — which test, the attempts made, and why RED cannot be established — and return control to `pipeline`. A PRESERVATION test that fails before implementation is **diagnosed**, never categorically treated as a test defect: (1) **test defect** — the expectation, fixture, or setup is wrong: correct it within the same two-correction-attempt budget, re-run, and record PASS; (2) **baseline failure** — the test correctly expresses the constraint and the current code does not satisfy it: retain the test and its observed failure unchanged, never weaken the expectation, record `PRESERVATION_BASELINE_FAILED` in RED-REPORT.md's Preservation results with the evidence, and return control to `pipeline`; (3) **environment** — record `RUNNER_UNAVAILABLE` (see STOP conditions). The developer's decision on a `PRESERVATION_BASELINE_FAILED` STOP (reclassify the test WITNESS, exclude it from the contract, or abort) is recorded by `tester` in TEST-CONTRACT.md's Test classification with the reason (this is a stage-5 write made before the contract is handed forward, not an amendment); on reclassify, `tester` re-proves RED for that test as a WITNESS under the same bounded-correction rule.
...
`tester` cannot voice any of these itself — it records the condition (which test, attempts made, why RED cannot be established, the preservation diagnosis, or the environment condition) in RED-REPORT.md and returns control to `pipeline`, which voices the STOP verbatim; the developer decides whether the behavior already exists, the criterion is wrong, the test needs redesign, scaffolding is approved as scope, the plan is redirected, the run is aborted, the PRESERVATION test is reclassified WITNESS or excluded, or fixes the environment and resumes.
...
`tester` creates or updates only TEST-CONTRACT.md and RED-REPORT.md. PLAN.md is an immutable input. `tester` never edits IMPLEMENTATION.md, VERIFICATION.md, or any other artifact — including its own two artifacts outside the amendment procedure, which is the sole authorized path to change them after stage 5; recording a `PRESERVATION_BASELINE_FAILED` decision happens within stage 5, before hand-forward.
