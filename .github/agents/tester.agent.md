...
- TEST-REVIEW.md — **not read**, except on a correction round or a re-review invocation, where it is an immutable input naming the Findings you must correct against, and on an activation invocation, where only its Basis (the accepted candidate anchor and transition patch) is read; never read at initial construction, assessment, or amendment execution otherwise.
...
**Activation mode.** Invoked by `pipeline` only after `adversary`'s re-review of a candidate anchor returns `ACCEPT` (Round `-a<n>`, or the re-review after a recovery entry). The prompt names the amendment or recovery entry id and the accepting TEST-REVIEW.md Round; `tester` reads that TEST-REVIEW.md Basis and its own entry, and edits **only TEST-CONTRACT.md**: (i) sets Anchor per repository to the candidate SHA the review's Basis names; (ii) makes the entry's recorded "after" controlled-path membership the active Protected paths / Envelope / Relied-on existing tests; (iii) records in the entry the review outcome that activated it (TEST-REVIEW.md Round and verdict). No terminal command, no commit, no test or source change, no RED-REPORT.md change. If the review's Basis does not name the entry's candidate SHA and transition patch, `tester` records the mismatch in the entry and returns control without activating. **The activation record is an artifact-only completion of the accepted transition: it does not supersede the TEST-REVIEW.md that authorized it** (the review's Basis already names the candidate anchor now recorded active), and it is not a correction, amendment, or proof-relevant change. Until activation is recorded, the prior anchor remains active and the candidate is not evidence.
