...
9. **Publish.** If G4 is `PUBLISH_AND_PR` or `PUBLISH_ONLY`, invoke `workspace` for stage 9. Relay any of `G4_NOT_RECORDED`, `REVERIFICATION_REQUIRED`, `PUSH_FAILED`, `REMOTE_SHA_MISMATCH` verbatim. If G4 is `ABORT`, do not invoke `workspace` or `pr`.
...
- **G4 — Publish and PR.** "Verification passed for `<repos>` at `<SHAs>`. Publish these branches and open the pull request(s)?" Options: Approve publish and PR / Approve publish only / Abort. Record the answer as exactly one of `PUBLISH_AND_PR`, `PUBLISH_ONLY`, `ABORT`, the SHAs shown, and a timestamp only if a tool produced one. Stage 9 runs for either `PUBLISH_AND_PR` or `PUBLISH_ONLY`; stage 10 runs only for `PUBLISH_AND_PR`.
