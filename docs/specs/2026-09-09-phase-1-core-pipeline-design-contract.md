...
## Lean Contract (Claude → Sonnet handoff)
...
- Sonnet implements each checkpoint; Claude reviews independently against every acceptance criterion; at most two correction rounds per checkpoint; Claude commits on PASS.
...
**R2 — Agents.**
Files: the nine `.agent.md` files; `.vscode/settings.json` extended with the tester, developer, and publish git forms (`git -C <dir> add <path>`, `git -C <dir> commit -m <msg>`, `git -C <dir> diff …`, `git -C <dir> log …`, `git -C <dir> push -u origin <branch>`), reviewed offline by Claude, no test log.
...
**2026-09-10 (owner, post-ChatGPT remediation; commits follow R3 `7a0861b`).** Five amendments arising from the independent architecture audit and the owner-approved Claude remediation plan. They correct evidence bindings, test-integrity rules, recovery semantics, and security claims. They add no runtime, schema, state machine, or script; every change is a contract statement, an agent procedure step, an allowlisted read-only git form, or an artifact template section. The nine roles, eleven artifacts, and every decision listed under "What we will not change" in the remediation plan stand.
...
**2026-09-10 (owner, post-closure-review targeted corrections; commits follow fix-4 `c2f7036`).** Three amendments arising from the independent remediation closure review (`docs/reviews/2026-09-10-phase-1-remediation-closure-review.md`, findings R1–R6) and the owner-approved Claude evaluation. They bind human approvals to what was actually reviewed, make existing-PR reuse meet the same guarantees as creation, and stop a failing preservation test from being normalised as a test defect. They add no runtime, schema, state machine, or script; every change is a contract statement, an agent procedure step, an artifact template field, a STOP catalogue entry, or a tightened approval-rule slot. The nine roles, eleven artifacts, and every decision under "What we will not change" stand.

**A8 — Gate answers are bound to the artifact revision they approved (amends §5 stage 0 and A6's resume rule; closes R1).**
- Every gate answer in RUN.md's gates log records its **basis** — the artifact revision(s) the developer actually reviewed when answering — and a **status** of `CURRENT` or `STALE`. Bases: G1 and G2 → INTAKE.md round; G3 → PLAN.md round and ADVERSARY-REVIEW.md round; G4 → VERIFICATION.md round, PR-DESCRIPTION.md round, and the per-repository tuple (A3).
- In the same RUN.md edit that marks an artifact SUPERSEDED in the Artifact history (an authorized A6 regeneration or a developer-decided redo), `pipeline` marks every gate answer whose basis names that artifact `STALE`. A `STALE` answer is never treated as an approval; it stays in the log for lineage.
- Before invoking any stage that follows a gate, `pipeline` requires a `CURRENT` answer for that gate whose basis equals the ACTIVE round of each artifact it names; otherwise it re-asks the gate, recording a new `CURRENT` entry. The resume point becomes the earlier of (a) the first missing or failed artifact and (b) the first gate in G1–G4 with no `CURRENT` answer (none recorded, or `STALE`).
- Consequences: the ordinary adversary REVISE → planner round 2 precedes G3, so no G3 answer exists to go stale; a developer-decided plan redo after G3 stales G3; a tester amendment supersedes TEST-CONTRACT.md and RED-REPORT.md only, so G3 stays `CURRENT`; a verifier re-run after G4 stales G4, and the stage 9 preflight (`REVERIFICATION_REQUIRED`, `G4_STALE`) remains the mechanical backstop behind it.

**A9 — Existing pull request eligibility (amends A3's PR-agent bullet; closes R2).**
- Stage 10 eligibility is checked per repository **before** any Bitbucket read for that repository: VERIFICATION.md is PASS; G4 is `CURRENT` and `PUBLISH_AND_PR`; WORKSPACE.md's remote SHA equals the verified SHA; the target branch is the default branch WORKSPACE.md recorded (equal to the G4 target). A repository failing any condition is reported blocked and receives neither a created nor a reused PR.
- The current remote source-branch SHA is re-read through the Bitbucket read capability on **both** paths, immediately before creating and immediately before recording an existing PR as reused.
- An existing PR is recorded `REUSED_EXISTING` only when all hold: same repository; source branch = G4 source branch; target branch = G4 target branch; state OPEN; re-read remote SHA = verified SHA. Otherwise `pr` records `EXISTING_PR_MISMATCH` naming every mismatching field (target, state, SHA), creates nothing for that repository, never modifies the existing PR, and reports the blocking condition through `pipeline`; the developer resolves it outside the pipeline. A source-branch name match alone is never fulfilment.
- PR.md records per repository an outcome of `CREATED`, `REUSED_EXISTING`, or `BLOCKED (<condition>)`. "Existing PR found" is therefore not itself a blocking condition; a mismatching existing PR is.

**A10 — Preservation-test diagnosis (amends A4's witness/preservation bullet and §5 stage 5; closes R4).**
- A PRESERVATION test that fails before implementation is **diagnosed**, never categorically treated as a test defect. `tester` establishes, with evidence recorded in RED-REPORT.md's Preservation results, which one holds: (1) **test defect** — the expectation, fixture, or setup is wrong: correct the test within the same two-correction-attempt budget, re-run, record PASS; (2) **baseline failure** — the test correctly expresses the constraint and the current code does not satisfy it (a pre-existing defect or an unmet criterion): retain the test and its observed failure unchanged, never weaken the expectation, record `PRESERVATION_BASELINE_FAILED` in RED-REPORT.md, and return control to `pipeline`; (3) **environment** — `RUNNER_UNAVAILABLE`.
- STOP `PRESERVATION_BASELINE_FAILED` is decided by the developer, one STOP and one decision per occurrence, never looped: reclassify the test WITNESS (the constraint becomes missing behavior in scope; its failure is then legitimate RED and it is implemented and verified like any WITNESS), exclude the test from the contract (recorded in TEST-CONTRACT.md with the developer's reason; the observed failure stays in RED-REPORT.md), or abort. Either recorded outcome is written by `tester` in TEST-CONTRACT.md's Test classification with the reason and the developer's decision.
- The verifier's expectations are unchanged (WITNESS now GREEN, PRESERVATION still GREEN; a full-suite failure caused by the implementation is FAIL).
- Wording: the amendment procedure's "new RED commit SHA" reads "new active anchor SHA" everywhere; an amendment anchor is the commit holding the amended tests, whatever their observed result.

**A9 clarification (owner, 2026-09-10, after the final closure review; commit fix-7).** The re-read required by A9's second bullet is a *comparison*, not only a read: after the fresh read and before either creating a new PR or recording an existing one as reused, `pr` requires, on the common path, re-read remote source-branch SHA = VERIFICATION.md verified SHA = G4 verified SHA. A mismatch records `BLOCKED (SHA mismatch)` for that repository, performs neither operation, and is reported through `pipeline`; the developer decides outside the pipeline (typically re-verification). The reuse eligibility list therefore no longer repeats the SHA condition — it is satisfied before reuse is considered. A branch that moves after the final read remains the disclosed server boundary; a mismatch already returned by the read is never acted past. The worked example's AC5 is likewise corrected so the `UNREPORTED` terminal outcome applies whenever the delivery becomes terminal (`DELIVERED` or `PERMANENTLY_FAILED`) with an attempt still unreported, whether by budget exhaustion or by an earlier successful delivery, with a WITNESS test for the early-success case; this is a correction of the example's G3-confirmed policy decision, not of the Jira-sourced recording requirement.

**Corrections without a contract change (R3, R5, R6), listed for traceability.** R5: the orchestrator's subagent-call convention no longer names INTAKE.md as a `workspace` input, and MODEL-ROLES.md no longer claims zero injection exposure for `workspace` (artifacts remain untrusted data, A7). R6: every `.vscode/settings.json` value slot follows the literal-token policy — a ref slot is `[A-Za-z0-9][A-Za-z0-9._/-]*`, admitting no `$`, backtick, parenthesis, quote, or shell operator — and the independent-name `switch -c <pbranch> --track origin/<pbranch>` rule is removed as redundant with the backreference rule; unsupported shapes prompt. R3: the worked example decides the exhausted ledger-reporting outcome explicitly as a G3-confirmed product decision and covers the failure path with WITNESS tests, so no example artifact claims a guarantee its tests do not demonstrate.

## 15. Phase 1 reference closure

**2026-09-10 (owner; reviewed revision `03d4230e9398a80586a4b8be47ed522638bf77d8`, commit fix-7).** The independent R2/R3 closure review (`docs/reviews/2026-09-10-phase-1-r2-r3-closure-review-03d4230.md`, kept outside the tracked tree as an independent artifact like its two predecessors) found R2 CLOSED, R3 CLOSED_WITH_ACCEPTED_LIMITATION, no new material regression within the fix-7 change, and recommended READY_TO_LOCK_AS_REFERENCE. Claude independently re-traced both findings against the source at that revision (PR-agent procedure steps 4–7, AGENT-CONTRACTS `pr`, FLOW stage 10, the A9 clarification; the worked example's PLAN AC5, RUN G3, ADVERSARY-REVIEW, TEST-CONTRACT, RED-REPORT, IMPLEMENTATION, VERIFICATION, PR-DESCRIPTION, and PR.md) and confirmed the recommendation; the earlier stale "all four" envelope-check count in VERIFICATION.md is corrected to seven. Local HEAD and `origin/main` were both queried and resolve to that revision.

Phase 1 is therefore **closed as the reference baseline at `03d4230`**. The annotated tag `phase-1-reference` pins that revision. Every reference asset — the nine agents, three skills, four pipeline documents, `copilot-instructions.md`, `.vscode/settings.json`, the worked example, and the comparison guide — is unchanged since that revision; this section and the README status line are the only later edits.

**Accepted limitations carried into the closure (unchanged).** Artifact lineage, diagnosis, and command/path restrictions are procedural and model-mediated where the host does not enforce them. Test/build execution and Git hooks run repository-controlled code, which the reference neither sandboxes nor purges from ignored outputs. A remote branch can move after the final read; server protection governs that interval, and a mismatch already returned by the read is never acted past. Multi-repository publication is non-atomic. Regex approval is a best-effort backstop, a prompt rather than a prohibition. The worked example is fictional and demonstrates adequate evidence, not real execution. `UNREPORTED` makes the ledger-reporting gap observable locally in `payments-api` without guaranteeing eventual ledger persistence, reconciliation, or alerting. None of these requires a custom runtime, database, scheduler, cryptographic mechanism, or locking system.

**Deferred corporate validation (still NOT VERIFIED, unchanged from §12).** Exact Jira and Bitbucket MCP tool names; the exact Sonnet-5 model-picker string; the Copilot Chat agent-picker smoke check; corporate approval-engine behavior; broader model benchmarking. Reference readiness and validation inside the corporate Copilot environment remain separate conclusions. This closure claims no production or corporate-environment validation and does not authorize Phase 2.
