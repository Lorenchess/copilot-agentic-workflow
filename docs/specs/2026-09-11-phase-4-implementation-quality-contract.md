...
- **Source.**
  - Revision 1 applied ChatGPT's architecture review (`docs/reviews/2026-09-11-phase-4-architecture-review.md`: PROCEED_WITH_CHANGES, material findings M1–M5, D1–D3 ACCEPT_WITH_CHANGES, D4 ACCEPT) to the proposal `docs/specs/2026-09-11-phase-4-implementation-quality-architecture-proposal.md`.
  - Revision 2 applies ChatGPT's review of revision 1 (`docs/reviews/2026-09-11-phase-4-contract-review.md`: TARGETED_CORRECTION_REQUIRED, corrections C1–C3 plus four precision notes; the reviewed revision 1 had SHA-256 `CFC8B3F8…7533`).
...
## 7. Closure evidence — challenge cases (traced at P4-C3)

| # | Case | Expected outcome | Rules | Document trace |
|---|---|---|---|---|
| 1 | The contract command is narrowed, or a skip/failure-ignore property is added | Not GREEN; the Verifier runs the exact command; any envelope hunk → FAIL | IQ2, IQ8 | **Document trace; not exercised.** `implementation-quality` IQ2 rejects command/discovery/configuration games; `developer.agent.md` requires the exact command, and `verifier.agent.md` steps 4 and 7 independently run it and inspect envelope hunks. |
| 2 | Production code special-cases a fixture literal | FAIL (GAMING); scoped fix round | IQ2, IQ12 | **Document trace; not exercised.** IQ2 names fixture overfitting; IQ12 makes `GAMING` blocking and routes a production-only scoped fix with full re-verification. |
| 3 | Production code detects the test profile | FAIL (GAMING); fix round; no controlled path involved, so no restoration | IQ2, IQ12 | **Document trace; not exercised.** IQ2 names test-aware production code; IQ12 separates `GAMING` from `CONTROLLED_PATH`, so `pipeline.agent.md` sends a fix round rather than human restoration. |
| 4 | A contract identity flips and then passes on a rerun | Diagnosed; never laundered | IQ2, IQ4 | **Document trace; not exercised.** IQ2 forbids flake laundering; IQ4 classifies the flip `NONDETERMINISTIC` and routes it by production, test, or environment cause. |
| 5 | A hard-coded value violates C1 while every test passes | FAIL (DEVIATION) → fix round → PASS (LARGE example) | IQ11, IQ12 | **Exercised by the fictional LARGE example.** Round 1 of `PAYMENTS-12345-testing/IMPLEMENTATION.md` hard-codes one second while every named test is GREEN; VERIFICATION.md anchors F1 to C1 and the hunk as `DEVIATION`, then records F1 `CLOSED` and PASS after the scoped round-2 fix. |
| 6 | A plan-implied adapter is missing from Affected files | `PLAN-TRACED` with necessity; an evidence row alone never suffices (LARGE example) | IQ5 | **Exercised by the fictional LARGE example.** IMPLEMENTATION.md traces `LedgerClient`/`RestLedgerClient` to the Approach and I1 and states why the adapter edit is necessary; VERIFICATION.md accepts that classification from the full inventory and hunk review. |
| 7 | Unrelated cleanup, or a whole-file reformat of a planned file | FAIL (SCOPE) | IQ5, IQ7 | **Document trace; not exercised.** IQ5 requires every hunk in a planned/traced path to connect to an approved element; IQ7 forbids opportunistic renames, formatting, upgrades, and cleanup. |
| 8 | Delivering the contract needs an unplanned persisted field | `IMPLEMENTATION_BLOCKED (MATERIAL_DEVIATION)` → planning | IQ6 | **Document trace; not exercised.** IQ6 makes a new persisted shape `MATERIAL`; `developer.agent.md` records BLOCKED and `pipeline.agent.md` offers planning, a directed in-scope round, or abort without approving the deviation inline. |
| 9 | Developer loops on diagnostic runs without running the contract | The diagnostic allowance is spent after six; further progress needs contract iterations, which are counted and bounded | IQ3 | **Document trace; not exercised.** IQ3 gives diagnostics and contract iterations independent reservation-first counters; the sixth diagnostic consumes that allowance and cannot be presented as GREEN. |
| 10 | Interruption mid-round, then resume | Same round, remaining allowance; a reservation without a result is `UNKNOWN` and consumed; no renewal | IQ3 | **Document trace; not exercised.** IQ3 defines reservation-first interruption accounting; the identical resume sentence in `pipeline.agent.md`, FLOW.md, and `pipeline/SKILL.md` resumes an `IN_PROGRESS` IMPLEMENTATION.md in the same round. |
| 11 | Resume while a BLOCKED decision is pending | The STOP is re-voiced; `developer` is not invoked | IQ3 | **Document trace; not exercised.** The shared resume sentence treats BLOCKED without a recorded decision as pending and requires `pipeline` to re-voice `IMPLEMENTATION_BLOCKED` without dispatching Developer. |
| 12 | Handoff attempt `H1` finds a new full-suite regression while allowance remains | Classified `REGRESSION`, not `BASELINE_FAILURE`; that candidate is not handed off; repaired within the remaining allowance; `H2` → GREEN | IQ3, IQ4, IQ14 | **Document trace; not exercised.** IQ4's candidate-versus-round rule keeps the round open; IQ14 defers to `REGRESSION` repair and IQ3 permits H2 while the handoff allowance remains. |
| 13 | Same-locus failure in an unchanged test that runs through a changed shared dependency | The diff touches its observation path, so it is `REGRESSION`, never `BASELINE_FAILURE`; repaired within the allowance, or `BUDGET_EXHAUSTED`; stage 7 FAILs it regardless; no PASS (D3(c)) | IQ4, IQ11, IQ14 | **Document trace; not exercised.** IQ4 disqualifies baseline routing when a changed path lies on the observation path or Developer is uncertain; IQ14 blocks every stage-7 failing identity under D3(c). |
| 14 | A failing identity that failed identically in the recorded baseline, with no changed path on its observation path | `IMPLEMENTATION_BLOCKED (BASELINE_FAILURE)`, with no claim about cause; the human directs a round, sends the plan back, or aborts | IQ4, IQ14 | **Document trace; not exercised.** IQ4 requires matching identity/assertion/result plus an unchanged observation path and forbids a causal claim; the STOP options live in `developer.agent.md` and FLOW.md. |
| 15 | Developer weakens a protected test in the working tree while HEAD is unchanged | The check's working-tree half fails → `CONTROLLED_PATH_CHANGED`. Human restoration, which may need no commit. The new round records the observed HEAD and passes both halves before any runner execution | IQ2, IQ4, D2 | **Document trace; not exercised.** IQ2's status half covers staged, unstaged, deleted, and renamed controlled paths; IQ4/D2 reserve restoration for the human and require a fresh two-half check before execution. |
| 16 | The Verifier finds a controlled-path change | `CONTROLLED_PATH` → the STOP instead of an automatic fix round | IQ12 | **Document trace; not exercised.** `verifier.agent.md` records a blocking `CONTROLLED_PATH` finding; `pipeline.agent.md` replaces the automatic fix round with `IMPLEMENTATION_BLOCKED (CONTROLLED_PATH_CHANGED)` and human restoration. Any resulting verification-origin directed round still counts against the unchanged fix budget. |
| 17 | A planned change logs a request object containing a token | FAIL (DEFECT) under the security baseline | IQ12 | **Document trace; not exercised.** IQ12's narrow `DEFECT` route accepts a specific hunk, token-exposure mechanism, and sourced security baseline even when no plan id states the invariant. |
| 18 | A defect needs a new product decision | `DEFECT — decision required` → planning | IQ12 | **Document trace; not exercised.** IQ12 forbids the Verifier from inventing a product decision; the finding returns through `MATERIAL_DEVIATION` to planning. |
| 19 | One repository GREEN, the other blocked | No handoff; per-repository status in the STOP | IQ9 | **Document trace; not exercised.** IQ9 requires every repository GREEN and preserves each repository's status/commits when one blocks; `IMPLEMENTATION_BLOCKED` names the full status set and nothing is pushed. |
| 20 | An envelope justification omits a hunk | FAIL (ENVELOPE) | IQ8 | **Document trace; not exercised.** IQ8 requires per-hunk justification for ordinary envelope changes; the Verifier's envelope diff reconciles each hunk and FAILs the omission. |
| 21 | A fix round adds an unrelated path | FAIL (SCOPE) | IQ13 | **Document trace; not exercised.** IQ13 limits the round to finding fix scopes plus declared consequential edits; the fresh full inventory classifies any other path `UNRELATED`. |
| 22 | A `NOT_VERIFIED` clause is left unimplemented vs implemented | FAIL vs `INCOMPLETE` | IQ11 | **Document trace; not exercised.** IQ11 requires an implementation location: absent is FAIL, present remains `INCOMPLETE`; `verifier.agent.md` preserves both outcomes and bars `INCOMPLETE` from G4. |
| 23 | Only an uncommitted file makes the contract pass | The handoff bracket and post-commit run catch it; the Verifier's checkout check also catches it | IQ2 | **Document trace; not exercised.** IQ2 requires the passing contract run inside a clean committed-HEAD bracket; `verifier.agent.md` independently checks clean state and matching SHA before and after execution. |
| 24 | A rollout-only obligation | `ROLLOUT` disclosure at G4 (LARGE example) | IQ11 | **Exercised through stage 7 by the fictional LARGE example.** IMPLEMENTATION.md records ledger-first deployment as `ROLLOUT`; VERIFICATION.md carries it in Disclosures while allowing PASS. G4 itself is not exercised because the example stops at stage 7; the pipeline documents require the disclosure there in a real continuation. |
| 25 | A SMALL change | Compact artifacts; one-pass PASS (SMALL example) | IQ15 | **Exercised by the fictional SMALL example.** `PAYMENTS-12410/IMPLEMENTATION.md` is reported as 50 lines, records one contract iteration and H1, and its VERIFICATION.md records a compact zero-blocker PASS without weakening IQ2/IQ12 controls. |
| 26 | The sixth contract iteration is the first to pass | It stands; handoff attempt `H1` may start | IQ3 | **Document trace; not exercised.** IQ3 checks a limit before a step and never overwrites the completed step's outcome; independent counters leave H1 available. |
| 27 | Handoff attempt `H2` succeeds completely | The round completes `GREEN`; no `BUDGET_EXHAUSTED` | IQ3 | **Document trace; not exercised.** IQ3 permits two attempt ids and states explicitly that success on the second allowance completes GREEN. |
| 28 | A handoff attempt is interrupted after its contract run | The reserved component is `UNKNOWN`; the attempt counts as failed; resume continues the same round with the remaining allowance | IQ3 | **Document trace; not exercised.** IQ3 counts H1 when it starts and marks its result-less reserved component `UNKNOWN`; the resume rule retains the same round and the remaining single handoff attempt. |
...
## 10. Phase 4 checkpoint closure (2026-09-11)

**Implemented through P4-C3.** P4-C1 is commit `d525456` (`implementation-quality`; Developer, Verifier, and Pipeline procedures; FLOW.md, AGENT-CONTRACTS.md, GUARDRAILS.md, MODEL-ROLES.md, and the pipeline skill). P4-C2 is commit `a5af172` (the LARGE and SMALL examples extended through stage 7, including IMPLEMENTATION.md and VERIFICATION.md and the LARGE DIFF-EXCERPTS.md teaching artifact), followed by the bounded four-file correction `713a302` (complete active-anchor controlled-path comparisons and linked Phase 3 tagged views). P4-C3 is the commit containing this section (comparison guide, README, all §7 traces, and this checkpoint record). No Phase 5 asset is included.

**Checkpoint provenance.** The P4-C1 and original P4-C2 commits retain their recorded Sonnet-5/Claude implementation and supervision history, including P4-C2's first Sonnet correction before `a5af172`. After ChatGPT's independent P4-C2 review, GPT-5.6 Sol authored the second, four-file C2 correction and this P4-C3 change under ChatGPT's supervision; ChatGPT verified the Sol worker's actual `gpt-5.6-sol`/high runtime selection from platform turn-context metadata rather than relying on self-identification. This authoring provenance does not change the reference pipeline's model-role contract: Sonnet 5 remains assigned to every pipeline role, with exact picker identity and benchmarks still deferred.

**Acceptance status.** P4-C1 was independently accepted at `d525456` with N1–N3 below. P4-C2 was independently accepted after `713a302`. P4-C3 implements §6's guide, README, trace, and closure scope: the guide includes current Phase 4 agent/skill/pipeline/example entries and the theme "Evidence-bound GREEN and anchored verification"; the Phase 3 N1 stale eleven-artifact, bounded-test-review, and resume summaries are corrected; README is current; and all 28 §7 cases are traced. Cases 5, 6, 24, and 25 are exercised by the fictional examples; every other row is explicitly a document trace. Case 24 exercises the stage-7 `ROLLOUT` record and disclosure, not G4, because both examples stop at stage 7. Independent P4-C3 and assembled Phase 4 review remain pending; this section does not claim final PASS, a locked reference, publication, or a tag.

**Owner decisions and later authorization.** D1–D4 were adopted as qualified: the bounded logical-round defaults and one new conditional STOP; human-only controlled-path restoration; D3(c), under which every full-suite failure blocks and qualified PASS stays deferred; and the in-place examples. The contract header records those decisions and the then-current P4-C1 authorization, while §8 retains its earlier pending-recommendation wording. A later explicit owner handoff accepted C1 and authorized the remaining P4-C2 and P4-C3 work, checkpoint closure, and publication after acceptance, prospectively superseding the header's hold on that remainder and §8's pending state without rewriting either historical record. Publication still waits for the independent assembled acceptance above.

**Accepted non-blocking debt.** N1 — P4-C1's operative rules agree, but detailed accounting and routing text remains normatively repeated in Developer/AGENT-CONTRACTS and the policy skill still names this contract as authority on divergence, so §6(a)'s literal "exactly one normative place" is qualified; future consolidation must preserve the agents' obligation to apply the skill. N2 — third-FAIL precedence versus a `CONTROLLED_PATH` or decision-required finding is not explicit; the controlling interpretation is no extra fix round or allowance renewal, while required human restoration and the special finding are still disclosed. N3 — VERIFICATION.md's envelope template summarizes justification-or-FAIL less precisely than the operative distinction between ordinary `ENVELOPE` and proof-relevant/relied-on `CONTROLLED_PATH` routing. These are documentation/maintenance notes, not permission for an alternative transition, and no additional operating-file correction is made in P4-C3.

**Not validated or closed here.** The examples are fictional document scenarios and no described test, build, git, agent, or pipeline command was executed for them. The procedural guarantees do not mechanically establish model compliance, evidence fidelity, approval-engine behavior, or protection against concurrent external edits; observation-path classification remains model judgment, and the 6/2/6/2 allowance defaults remain unmeasured. Corporate Copilot execution, exact Jira/Bitbucket MCP names, the Sonnet-5 picker string, the agent-picker smoke check, model benchmarks, and the other §9 items remain deferred. Final assembled review, any administrative reference-closure section, publication, and any new reference tag are later steps.

## 11. Phase 4 reference closure

**Final acceptance (2026-09-11).** ChatGPT's independent assembled review (`docs/reviews/2026-09-11-phase-4-final-implementation-review.md`, retained outside the tracked tree) reviewed exact revision `9a7e704cf328b161b08cc0e903a883ef340d4368`, found zero material findings and zero material regressions, and returned **PASS — READY_TO_PUSH_AND_LOCK_PHASE_4**. P4-C1 remains accepted at `d525456f8f98c8f9d6c63f726b2c8f2e3ffa6fe6`; P4-C2 is `a5af1725197922d3cbacf2e7959ee2703c170606` plus its accepted correction `713a302103bc2b02d00949d78f0eeefe0399c1d8`; P4-C3 is the reviewed revision `9a7e704cf328b161b08cc0e903a883ef340d4368`.

**Reference target and publication state.** The accepted Phase 4 reference content is exactly `9a7e704cf328b161b08cc0e903a883ef340d4368`; the annotated tag `phase-4-reference` must target that revision, never this later administrative-closure commit. The owner's takeover instruction is the authority for closure and publication after acceptance; the review recommendation confirms readiness but does not itself grant that authority. At the time this section is written, the administrative commit, main push, tag creation/push, and published-ref re-read have not yet been performed or verified, and no remote publication is claimed.

**Authoring and review provenance.** Historical P4-C1 and original P4-C2 Sonnet/Claude authorship, including C2's first correction, remains as recorded and is not retroactively certified. After the owner takeover, GPT-5.6 Sol made C2's second correction and every C3/administrative edit under ChatGPT's supervision; ChatGPT verified `gpt-5.6-sol`, effort `high`, from platform turn-context metadata, reviewed the actual files and diffs, and performed the accepted Git operations. C2's second correction used the remaining checkpoint budget rather than restarting it. ChatGPT's assigned supervisor/reviewer role is independent of the implementation worker's claims, not a blind external audit. The reference pipeline itself still assigns Sonnet 5 to every runtime role.

**Historical status and accepted debt.** The header records the earlier owner approval of D1–D4 and P4-C1 authorization; §8 preserves the still-earlier pending recommendations; §10 records the later takeover and the then-pending assembled review. The owner's prospective authorization and the final acceptance above supersede those holds without rewriting them. P4-C1 notes N1–N3 remain accepted, not fixed: detailed normative repetition qualifies §6(a)'s single-home criterion; third-FAIL/special-STOP precedence grants no extra fix round but remains textually ambiguous; and the VERIFICATION envelope-template summary remains less precise than the operative `ENVELOPE`/`CONTROLLED_PATH` routing. Phase 3's guide N1 is fixed by P4-C3, while its historical closure-attribution N2 remains preserved.

**Limits carried forward.** The examples and §7 traces are fictional document scenarios, not executed fault-injection or corporate Copilot validation. Budgets, evidence fidelity, reading order, path classification, model compliance, status/diff observations, and same-model independence remain procedural; the allowance defaults and manual-approval cost are unmeasured, ignored/generated state may influence execution, and an already-failing full suite still blocks completion. Corporate Jira/Bitbucket tool names, the Sonnet-5 picker identity, agent-picker smoke check, approval-engine behavior, model benchmarks/diversity, and the remaining §9 items stay deferred. Phase 5 is not authorized.
