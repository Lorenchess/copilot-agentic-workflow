...
**Authoring and review provenance.** Historical P4-C1 and original P4-C2 Sonnet/Fable authorship, including C2's first correction, remains as recorded and is not retroactively certified. After the owner takeover, GPT-5.6 Sol made C2's second correction and every C3/administrative edit under Astra's supervision; Astra verified `gpt-5.6-sol`, effort `high`, from platform turn-context metadata, reviewed the actual files and diffs, and performed the accepted Git operations. C2's second correction used the remaining checkpoint budget rather than restarting it. Astra's assigned supervisor/reviewer role is independent of the implementation worker's claims, not a blind external audit. The reference pipeline itself still assigns Sonnet 5 to every runtime role.
