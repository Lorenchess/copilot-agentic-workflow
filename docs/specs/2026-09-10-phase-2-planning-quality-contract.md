...
**Resume and supersession with planning cycles** (clarifies A6/A8 for R8, no new mechanism): a `SEND_BACK` G3 entry is never an approval, so for the pre-stage rule and the resume rule "a `CURRENT` answer" for G3 means a `CURRENT` `APPROVE`; starting a cycle marks the prior PLAN.md and ADVERSARY-REVIEW.md SUPERSEDED in the same RUN.md edit that records the `SEND_BACK` or redirection, an artifact whose versions are all SUPERSEDED counts as **missing** for resume (so an interrupted send-back resumes at stage 3, never at stage 5), and the only agent that reads a SUPERSEDED artifact is `planner`, reading the immediately prior PLAN.md/ADVERSARY-REVIEW.md revision as the prior plan on a revise round or new cycle. Investigation notes in Affected files are written as prose (question plus evidence id), never as a path or path prefix, so the unchanged verifier's path match cannot treat them as operative. When a developer's redirection adds a repository, `pipeline` re-asks G1 for that repository (new `CURRENT` G1 entry) and runs stage 2 for it before the new planning cycle.
