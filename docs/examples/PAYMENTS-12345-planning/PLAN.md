...
Searched: `WebhookDeliveryService`, `WebhookDeliveryStatusController`, `LedgerClient`, `WebhookDelivery`, `application.yml`, "retry", "backoff", "Scheduler" [REPO]/[INFERENCE].
...
- Whether the existing `audit_log` table's columns already accommodate an attempt number distinctly, or whether a new endpoint alone suffices without a column addition — see E9, Q5.
...
Source class: JIRA (PAYMENTS-12345 Acceptance Criteria field). Observed at: the persisted `WebhookDelivery` status field via the existing repository (E4). Preconditions: the delivery has already failed the configured maximum number of attempts. Path: NEGATIVE.
...
|---|---|---|---|---|---|---|
| Q1 | Shipped defaults for `maxAttempts` and `baseDelay` are not stated in either Jira issue (INTAKE.md, Unknowns). | G3 | `maxAttempts = 5` total attempts, including the first; `baseDelay = 1` second (backoff delays before attempts 2–5: 1s, 2s, 4s, 8s). | INTAKE.md Unknowns; R6 shipped-value rule (inventing a value is `G3`). | Too few attempts (e.g. 3) abandons deliveries that would have recovered by attempt 5; too many or too-long delays (e.g. `baseDelay = 10s`) unacceptably lengthens merchant-visible delivery latency and audit-trail completion time. | G3 |
...
| Q4 | `LedgerClient` surfaces a failed report call as a thrown exception or another distinguishable failure result, rather than a silently success-shaped response. | ASSUMED | — (proceeding on this basis) | E2 — existing `LedgerClient` usage pattern elsewhere in `payments-api` [REPO]/[INFERENCE]. | If `LedgerClient` instead swallows failures and returns a success-shaped result, `WebhookRetryService` would treat a failed report as successful, never re-report it, and never mark `ledgerReportStatus = UNREPORTED` — the audit trail would silently lose records exactly as PAYMENTS-12351 requires it not to. Observable only if Phase 3's test double for a failing report fails loudly rather than mimicking silent success — a boundary condition for Phase 3 test design, not resolved here. | G3 (count) |
| Q5 | The existing `audit_log` table's schema already has columns sufficient for delivery id, attempt number, and outcome, so only a new write path/endpoint (not a schema change) is needed. | ASSUMED | — (proceeding on this basis) | E9; D2/C2 (developer instruction to reuse the existing table) [REPO]/[DEV]. | If the existing schema lacks a column this needs (e.g. a distinct attempt-number column), AC4 could only be satisfied by a persistence schema change, which is not currently modeled as a Dependencies/interfaces row and would need its own decision. | G3 (count) |
...
**Consequential assumptions.** 2 (Q4, Q5).
