...
| R2 | Bitbucket MCP server name as shown in the tools picker, and the exact tool names for: create pull request, read branch / read repository (to re-confirm the remote SHA before creating)? Needed to replace the commented-out capability lines in `pr.agent.md`'s `tools:` list with live entries. | Sonnet | Open | |
